...
//보드폴더랑 업로드 폴더 꼭 생성해놓기!!!!!!!	
	...
	public static Connection getConnection() { //리턴타입이 컨넥션 객체 
		...
		try {
			...
			System.out.println("connect success : JdbcUtil.java");
		}catch(Exception e) {
			e.printStackTrace();
		}
...
//전부 다 클래스 객체로 만들었기 때문에 변수가 없어도 호출이 가능함. 
	...
	public static void commit(Connection con) {
		...
		try {
			con.commit();
			System.out.println("commit success : JdbcUtil.java"); //중간에 찍어보는거 
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static void rollback(Connection con) {
		...
		try {
			con.rollback();
			System.out.println("rollback success : JdbcUtil.java");
		}catch (Exception e) {
			e.printStackTrace();
		}
	}
